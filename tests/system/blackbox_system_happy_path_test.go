...
var _ = Describe("System blackbox happy path", Ordered, func() {
	...
	It("uploads a real file over HTTP and completes the workflow via a real worker", func() {
		...
		signalNames, err := collectWorkflowSignalNames(context.Background(), temporalClient, upload.WorkflowID)
		Expect(err).ToNot(HaveOccurred())
		Expect(signalNames).To(ContainElement(appTemporal.ReviewDecisionSignalName))
